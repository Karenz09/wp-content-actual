(function($){ "use strict";
	...
	$(document).ready(function() {
		...
		// Init directory map
		if ( $.fn.lsvrPressvilleDirectoryMapGmaps ) {
			$( '.lsvr_listing-map--gmaps #lsvr_listing-map__canvas' ).each(function() {
				$(this).lsvrPressvilleDirectoryMapGmaps();
			});
		}
	...
	if ( ! $.fn.lsvrPressvilleDirectoryMapGmaps ) {
		...
		// Ajax request var
		var lsvrPressvilleDirectoryMapGmapsAjaxRequest;

		// Plugin methods
		var lsvrPressvilleDirectoryMapGmapsMethods = {
			...
			init : function() {
				...
				var apiSetup = function() {
			        ...
			        // Make ajax request
			        $this.lsvrPressvilleDirectoryMapGmaps( 'ajaxRequest', query );
			...
			ajaxRequest : function( query ) {
				if ( typeof query != 'undefined' ) {
			        ...
			        if ( typeof lsvr_pressville_ajax_directory_map_var !== 'undefined' ) {
						...
						if ( 'undefined' !== typeof lsvrPressvilleDirectoryMapGmapsAjaxRequest ) {
							lsvrPressvilleDirectoryMapGmapsAjaxRequest.abort();
						}
			        	lsvrPressvilleDirectoryMapGmapsAjaxRequest = jQuery.ajax({
			            	type: 'post',
			            	dataType: 'JSON',
			            	...
			            	success: function( response ) {
			            		...
			            		$this.lsvrPressvilleDirectoryMapGmaps( 'refresh', response );
		...
		// Plugin
		$.fn.lsvrPressvilleDirectoryMapGmaps = function( method ) {

			if ( lsvrPressvilleDirectoryMapGmapsMethods[ method ] ) {
				return lsvrPressvilleDirectoryMapGmapsMethods[ method ].apply( this, Array.prototype.slice.call( arguments, 1 ) );
			} else {
				return lsvrPressvilleDirectoryMapGmapsMethods.init.apply( this, arguments );
			}
