...
add_action( 'wp_ajax_nopriv_lsvr-pressville-toolkit-ajax-weather-widget', 'lsvr_pressville_toolkit_ajax_weather_widget' );
add_action( 'wp_ajax_lsvr-pressville-toolkit-ajax-weather-widget', 'lsvr_pressville_toolkit_ajax_weather_widget' );
if ( ! function_exists( 'lsvr_pressville_toolkit_ajax_weather_widget' ) ) {
	function lsvr_pressville_toolkit_ajax_weather_widget() {

		// Test nonce
		$nonce = ! empty( $_POST['nonce'] ) ? $_POST['nonce'] : false;
		if ( ! wp_verify_nonce( $nonce, 'lsvr-pressville-toolkit-ajax-weather-widget-nonce' ) ) {
			die ( esc_html__( 'You do not have permission to access this data', 'lsvr-pressville-toolkit' ) );
		}
		...
		if ( ! empty( $_POST['data'] ) ) {
			...
			$args_hash = 'lsvr_pressville_weather_widget_t_' . md5( serialize( $args ) );
			...
			if ( ! empty( $weather_response ) ) {
				...
				echo json_encode( array( 'error' => esc_html__( 'No response', 'lsvr-pressville-toolkit' ) ) );
			}

		} else {
			echo json_encode( array( 'error' => esc_html__( 'Missing data', 'lsvr-pressville-toolkit' ) ) );
		}
