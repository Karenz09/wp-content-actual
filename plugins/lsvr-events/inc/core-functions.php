		...
		if ( ! isset( $post_query_active ) ||
			...
			if ( ! empty( $retrieved_occurrences ) ) {
				...
				foreach ( $retrieved_occurrences as $key => $occurrence ) {
					...
					if ( 'publish' === get_post_status( $occurrence['postid'] )
						|| ( 'private' === get_post_status( $occurrence['postid'] ) && is_user_logged_in() ) ) {
						...
						unset( $retrieved_occurrences[ $key ] );
					}
				...
				if ( in_array( 'events', $to_return ) || in_array( 'event_id', $to_return ) ) {
				...
				if ( in_array( 'event_id', $to_return ) ) {
					$return['event_id'] = $retrieved_event_ids;
				}
...
/**
 * Determine the occurrence for the featured event
 */
if ( ! function_exists( 'lsvr_events_get_event_featured_occurrence' ) ) {
	function lsvr_events_get_event_featured_occurrence( $post_id ) {

        $upcoming_occurrence = lsvr_events_get_next_occurrences( $post_id );
        ...
        elseif ( empty( $upcoming_occurrence ) ) {

            $past_occurrence = lsvr_events_get_recent_occurrences( $post_id );
...
/**
 * Check if post has any taxnomy terms
 */
if ( ! function_exists( 'lsvr_events_has_post_terms' ) ) {
	function lsvr_events_has_post_terms( $post_id, $taxonomy ) {
...
/**
 * Display HTML with taxonomy terms
 */
if ( ! function_exists( 'lsvr_events_get_post_taxonomy_html' ) ) {
	function lsvr_events_get_post_taxonomy_html( $post_id, $taxonomy = 'lsvr_event_cat', $link_template = '<a href="%s">%s</a>' ) {
...
// Get maps provider
if ( ! function_exists( 'lsvr_events_get_maps_provider' ) ) {
	function lsvr_events_get_maps_provider() {
...
// Has maps provider
if ( ! function_exists( 'lsvr_events_has_maps_provider' ) ) {
	function lsvr_events_has_maps_provider() {

		return ! empty( lsvr_events_get_maps_provider() ) ? true : false;

	}
}

// Get calendar days
if ( ! function_exists( 'lsvr_events_get_calendar_days' ) ) {
    function lsvr_events_get_calendar_days( $year = false, $month = false ) {
    	if ( function_exists( 'lsvr_framework_get_calendar_days' ) ) {

    		return lsvr_framework_get_calendar_days( $year, $month );
...
if ( ! function_exists( 'lsvr_events_get_calendar_first_day' ) ) {
	function lsvr_events_get_calendar_first_day( $year = false, $month = false ) {
		if ( function_exists( 'lsvr_framework_get_calendar_first_day' ) ) {

			return lsvr_framework_get_calendar_first_day( $year, $month );
...
if ( ! function_exists( 'lsvr_events_get_calendar_last_day' ) ) {
    function lsvr_events_get_calendar_last_day( $year = false, $month = false ) {
		if ( function_exists( 'lsvr_framework_get_calendar_last_day' ) ) {

			return lsvr_framework_get_calendar_last_day( $year, $month );
...
if ( ! function_exists( 'lsvr_events_get_event_calendar' ) ) {
    function lsvr_events_get_event_calendar( $args = array( 'year' => false, 'month' => false, 'category' => false, 'location' => false, 'split_multiday_events' => false ) ) {
    	...
    	// Get calendar days
		$calendar_days = lsvr_events_get_calendar_days( $year, $month );

    	// Parse pre days
    	if ( ! empty( $calendar_days['pre'] ) ) {
    		foreach ( $calendar_days['pre'] as $day ) {
    			$event_calendar[ $day ] = array( 'status' => 'previous-month' );
    		}
    	}

    	// Parse currrent days
    	if ( ! empty( $calendar_days['current'] ) ) {
    		foreach ( $calendar_days['current'] as $day ) {
    			$event_calendar[ $day ] = array( 'status' => 'current-month' );
    		}
    	}

    	// Parse post days
    	if ( ! empty( $calendar_days['post'] ) ) {
    		foreach ( $calendar_days['post'] as $day ) {
    			$event_calendar[ $day ] = array( 'status' => 'next-month' );
    		}
    	}

    	// Get occurrences
    	$query_args = array(
    		'date_from' => lsvr_events_get_calendar_first_day( $year, $month ),
    		'date_to' => lsvr_events_get_calendar_last_day( $year, $month ),
			'orderby' => 'start',
			'to_return' => 'occurrences',
		);
		if ( ! empty( $args['location'] ) ) {
			$query_args['location'] = $args['location'];
		}
		if ( ! empty( $args['category'] ) ) {
    	...
    	// Parse occurrences
    	if ( ! empty( $occurrences['occurrences'] ) ) {
    		foreach ( $occurrences['occurrences'] as $occurrence ) {

				$occurrence_start_local = get_date_from_gmt( $occurrence['start'], 'Y-m-d' );
				$occurrence_end_local = get_date_from_gmt( $occurrence['end'], 'Y-m-d' );

    			// Save multiday events into multiple days
				if ( ! empty( $args['split_multiday_events'] ) && true === $args['split_multiday_events'] && ( $occurrence_start_local !== $occurrence_end_local ) ) {

        			$start_time = strtotime( $occurrence['start'] );
        			$end_time = strtotime( $occurrence['end'] );

        			for ( $i = $start_time; $i < $end_time; $i += 86400 ) {

        				$multiday_local = get_date_from_gmt( date( 'Y-m-d H:i:s', $i ), 'Y-m-d' );

        				if ( ! empty( $event_calendar[ $multiday_local ] ) ) {
        					$event_calendar[ $multiday_local ][ 'occurrences' ][] = $occurrence;
        				}
				...
				// Save multiday events only to start day
				elseif ( ! empty( $event_calendar[ $occurrence_start_local ] ) ) {
					$event_calendar[ $occurrence_start_local ][ 'occurrences' ][] = $occurrence;
				}
...
// Get month names
if ( ! function_exists( 'lsvr_events_get_list_of_month_names' ) ) {
    function lsvr_events_get_list_of_month_names() {

    	$month_names = array();
		for ( $i = 1; $i <= 12; $i++ ) {
			$month_names[] = date_i18n( 'F', strtotime( '01-' . str_pad( $i, 2, 0, STR_PAD_LEFT ) .'-2000' ) );
		}
